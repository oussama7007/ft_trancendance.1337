...
let mockListings: Listing[] = [
  {
    id: 1,
    title: { en: 'Modern Apartment in Agdal', fr: 'Appartement moderne à Agdal', ar: 'شقة عصرية في أكدال' },
    description: { en: 'Fully furnished apartment near all amenities.', fr: 'Appartement entièrement meublé près de toutes commodités.', ar: 'شقة مفروشة بالكامل قريبة من جميع المرافق.' },
    city: 'الرباط',
    cityEnFr: 'Rabat',
    district: 'أكدال',
    districtEnFr: 'Agdal',
...
let mockChats: ChatConversation[] = [
  {
    ...
    messages: [
      { sender: 'other', text: 'Salam, is the apartment still available?', time: '10:30 AM' }
    ]
  }
];
...
export const apiService = {
  ...
  createListing: async (newListingData: any): Promise<Listing> => {
    ...
    // معالجة البيانات سواء كانت جايين من FormData أو كائن عازي
    const titleText = newListingData instanceof FormData 
      ? newListingData.get('title') || 'عكار جديد' 
      : newListingData.title?.ar || newListingData.title || 'عقار جديد';

    const priceVal = newListingData instanceof FormData 
      ? Number(newListingData.get('price')) || 0 
      : Number(newListingData.price) || 0;

    const cityVal = newListingData instanceof FormData 
    ...
    mockListings.unshift(listingToAdd); // كيتزاد لفوق باش يتبان هو الأول
    return listingToAdd;
  },
