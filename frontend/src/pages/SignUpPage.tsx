...
const accentStyles: Record<Accent, { iconFocus: string; inputFocus: string }> = {
  orange: {
    iconFocus: 'group-focus-within:text-[#F4845F]',
    inputFocus: 'focus:border-[#F4845F] focus:bg-white focus:ring-4 focus:ring-[#F4845F]/10',
  },
  blue: {
    iconFocus: 'group-focus-within:text-[#3B82F6]',
    inputFocus: 'focus:border-[#3B82F6] focus:bg-white focus:ring-4 focus:ring-[#3B82F6]/10',
  },
  navy: {
    iconFocus: 'group-focus-within:text-[#1E293B]',
    inputFocus: 'focus:border-[#1E293B] focus:bg-white focus:ring-4 focus:ring-[#1E293B]/10',
  },
};
...
function getPasswordChecks(pw: string) {
  return [
    ...
    { label: 'Include uppercase and lowercase', met: /[a-z]/.test(pw) && /[A-Z]/.test(pw) },
    { label: 'Include number or special character', met: /[0-9]/.test(pw) || /[^A-Za-z0-9]/.test(pw) },
  ];
}

function getPasswordStrength(pw: string) {
  const score = getPasswordChecks(pw).filter((c) => c.met).length;
  const label = !pw ? '' : score <= 1 ? 'Weak' : score === 2 ? 'Fair' : 'Strong';
  const barColor = score <= 1 ? 'bg-red-400' : score === 2 ? 'bg-orange-400' : 'bg-green-500';
  const textColor = score <= 1 ? 'text-red-500' : score === 2 ? 'text-orange-500' : 'text-green-600';
  return { score, label, barColor, textColor };
}
  ...
  return (
    <div className="group">
      ...
      <div className="relative">
        <div className={`absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 ${isError ? 'text-red-400' : a.iconFocus} group-focus-within:scale-110 transition-all duration-200 pointer-events-none`}>
          {icon}
        </div>
        <input
          type={type}
          ...
          required={required}
          className={`w-full bg-gray-50/80 border ${isError ? 'border-red-400 bg-red-50/20 focus:border-red-500 focus:ring-4 focus:ring-red-500/10' : 'border-gray-200 hover:border-gray-300 hover:bg-white ' + a.inputFocus} rounded-2xl py-3.5 pl-12 pr-4 outline-none text-sm font-semibold text-gray-800 placeholder:text-gray-400 transition-all duration-200 focus:shadow-sm`}
        />
      </div>
    </div>
  );
        ...
        <div className="flex flex-col items-center gap-1.5">
          ...
          <span
            className={`text-[10px] font-bold uppercase tracking-wide whitespace-nowrap ${
              step === s.number ? 'text-[#F4845F]' : 'text-gray-400'
            }`}
          >
            {s.label}
          </span>
        ...
        {idx < STEP_META.length - 1 && (
          <div
            className={`h-0.5 w-10 sm:w-16 md:w-24 mb-5 mx-1 transition-all duration-300 ${
              step > s.number ? 'bg-[#F4845F]' : 'bg-gray-200'
            }`}
          />
        )}
      </React.Fragment>
...
/* =========================================================
   SIGN UP PAGE (MULTI-STEP WIZARD)
========================================================= */
...
export const SignUpPage: React.FC<SignUpPageProps> = ({
  ...
  // Unified identifier input state
  const [identifier, setIdentifier] = useState('');

  // Pro Error Message State
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setErrorMessage(null); // Clear error on submit attempt
    ...
    if (step < 3) {
      ...
      // Pro validation check for password match with animated error alert
    ...
    <div className="flex-1 relative overflow-y-auto px-4 py-10 md:px-8 bg-white">
      <div className="relative mx-auto w-full max-w-3xl bg-white rounded-[2rem] border border-gray-100 shadow-[0_15px_40px_rgba(0,0,0,0.08)] overflow-hidden">
        ...
        <div className="relative px-7 md:px-10 pt-9 pb-8 text-center border-b border-gray-100 bg-gradient-to-b from-orange-50/70 to-white">
          <div className="flex items-center justify-center gap-2 mb-4">
            <HouseIcon />
            <span className="text-xl font-black text-gray-900 tracking-tight">{t.appName || 'iRent'}</span>
          </div>

          <h2 className="text-3xl md:text-4xl font-black tracking-tight">
            <span className="text-gray-900">{t.signUpTitlePart1 || 'Create '}</span>
            <span className="text-[#F4845F]">{t.signUpTitlePart2 || 'Your Account'}</span>
          </h2>

          <p className="mt-2 text-sm text-gray-500 max-w-md mx-auto">
            {t.signUpSubtitle || 'Join us today and find your perfect place'}
          </p>
        ...
        <form onSubmit={handleSubmit} className="p-7 md:p-10">
              ...
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-400 mt-0.5">
                    Required information <span className="text-red-500 ml-1">*</span>
                  </p>
                </div>
              </div>
          ...
          {/* ================= STEP 2: UNIFIED IDENTIFIER (FACEBOOK/GOOGLE STYLE) ================= */}
          {step === 2 && (
            <div className="mb-8">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-400 mt-0.5">Enter your email or phone number</p>
                </div>
              </div>

              <div className="group">
                <label className="flex items-center gap-1.5 text-[11px] font-black uppercase tracking-wider text-gray-500 mb-2">
                  Email or Phone Number <span className="text-red-500 text-sm">*</span>
                </label>
                <div className="relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#3B82F6] transition-all duration-200 pointer-events-none">
                    {isEmailIdentifier || identifier.length === 0 || isNaN(Number(identifier.charAt(0))) ? (
                      <MailIcon />
                    ) : (
                      <PhoneIcon />
                    )}
              ...
              {/* Dynamic system detection alert */}
              ...
              <div className="group mt-5">
                <label className="flex items-center gap-1.5 text-[11px] font-black uppercase tracking-wider text-gray-500 mb-2">
                  Verification Code <span className="text-red-500 text-sm">*</span>
                </label>
          ...
          {/* ================= STEP 3: PASSWORD SETUP (WITH PRO ERROR ANIMATION) ================= */}
          {step === 3 && (
            <div className="mb-8 space-y-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-400 mt-0.5">Protect your account with a secure password</p>
                </div>
              </div>

              {/* PRO ANIMATED ERROR BANNER */}
              {errorMessage && (
                <div className="flex items-center gap-3 p-4 bg-red-50 border border-red-200 rounded-2xl text-red-600 animate-bounce duration-300 shadow-sm">
                  <AlertIcon />
              ...
              <div className="flex items-center gap-2 p-3.5 bg-gray-50 border border-gray-200 rounded-2xl">
                ...
                <span className="text-xs font-semibold text-gray-700">
                  {identifier} (Verified via {isEmailIdentifier ? 'Email' : 'Phone'})
                </span>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <FormInput
                    ...
                    onChange={(val) => {
                      setPassword(val);
                      if (errorMessage) setErrorMessage(null); // Clear error when typing again
                    }}
                    placeholder="••••••••"
                    accent="navy"
                  ...
                  {password && (
                    <div className="flex items-center gap-2 mt-2.5 px-1">
                      ...
                      <span className={`text-[10px] font-black uppercase ${strength.textColor}`}>
                        {strength.label}
                      </span>
                    </div>
                  )}
                ...
                <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl space-y-2">
                  {passwordChecks.map((c) => (
                    <div key={c.label} className="flex items-center gap-2">
                      ...
                      <span className={`text-[11px] font-medium ${c.met ? 'text-gray-700' : 'text-gray-400'}`}>
                        {c.label}
                      </span>
                    </div>
                  ))}
          ...
          <div className="flex items-center gap-2 mb-5 px-4 py-3 rounded-xl bg-red-50 border border-red-100">
            ...
            <span className="text-[11px] font-medium text-red-600">Fields marked with * are required</span>
          </div>
      ...
      <div className="flex justify-center mt-6">
        <div className="flex items-center gap-2 bg-white border border-gray-100 px-4 py-2 rounded-full shadow-sm">
          <LockIcon />
          <span className="text-xs font-semibold text-gray-700">Your data is safe with us</span>
        </div>
      </div>
    </div>
  );
