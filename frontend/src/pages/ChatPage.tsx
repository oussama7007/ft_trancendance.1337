      ...
      <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 flex-1 flex overflow-hidden">
            ...
            {chats.map((chat) => (
              ...
              >
                <div className="relative shrink-0">
                  <img src={chat.avatar} alt={chat.userName} className="w-12 h-12 rounded-2xl object-cover border border-gray-200 shadow-sm" />
                  <span className="absolute bottom-0 right-0 w-3 h-3 bg-emerald-500 border-2 border-white rounded-full"></span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-extrabold truncate text-gray-900">{chat.userName}</p>
                  <p className="text-[11px] text-gray-400 truncate mt-0.5 font-medium">{chat.lastMessage}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {activeChat ? (
          <div className="flex-1 flex flex-col bg-white">
            <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-white shadow-sm">
              <div className="flex items-center gap-3">
                <img src={activeChat.avatar} alt={activeChat.userName} className="w-10 h-10 rounded-2xl border border-gray-200 object-cover" />
                <div>
                  <h3 className="font-extrabold text-xs text-gray-900">{activeChat.userName}</h3>
            ...
            <div className="flex-1 p-5 overflow-y-auto space-y-3.5 bg-[#f8f9fa]">
              {activeChat.messages.map((msg, idx) => (
                ...
                >
                  <div className={`max-w-[75%] px-4 py-3 rounded-2xl text-xs leading-relaxed shadow-sm font-medium ${
                    msg.sender === 'me' 
                  ...
                  <span className="text-[10px] text-gray-400 mt-1 px-1 font-semibold">{msg.time}</span>
                </div>
              ))}
              <div ref={messagesEndRef} />
            </div>

            <form onSubmit={handleSendMessage} className="p-3.5 border-t border-gray-100 flex items-center gap-2 bg-white">
              <input 
                type="text" 
          ...
          <div className="flex-1 flex items-center justify-center text-gray-400 text-xs font-semibold">
            {t.selectChat || 'Select a chat conversation'}
          </div>
        )}
